...
This repository contains tools to simplify and format the output from CrowdStrike's Falcon Cloud Security Command Line Interface (FCS CLI) tool, making it more actionable and easier to integrate into CI/CD pipelines.

## Contents

- **`simplify_json.py`**: Python script that transforms FCS CLI scan results into a focused JSON format
...
```bash
fcs iac scan -p ./your-terraform-files/ -o ./
```
